                                  ...
                                  <div class="table_body">
                                    <ul class="product-list">
                                       ...
                                       foreach($adminrole as $value): ?>
                                            <li>
                                                <div class="inb">
                                                    <a href="View_profile?U=<?php echo base64_encode($value->id_user) ?>">
                                                   <?php if($value->image){ ?>
                                                    <img src="<?php echo base_url($value->image); ?>" alt="Starter Kit" class="rank-img">
                                                    <?php } else { ?>
                                                ...
                                                <button class="pull-right btn btn-custom p26 mt-11 roleupdate" data-id="<?php echo base64_encode($value->id_user); ?>" data-toggle="modal" data-target="#myModal"><i class="icon-pencil"></i></button>                                           
                                                <span class="pull-right stock-badge rank-badge out mt-15"><?php echo $value->role; ?></span>
                                            </li>
                                        <?php endforeach; } ?>
                                    </ul>
                                  </div>
...
<script type="text/javascript">
       $(document).ready(function () {
           $(".roleupdate").click(function (e) {
               ...
               $.ajax({
                   url: '<?php echo base_url();?>root/groupDataByID?id=' + iid,
                   method: 'GET',
                   data: '',
                   dataType: 'json',
               }).done(function (response) {
                   // Populate the form fields with the data returned from server
                $('#myModal').find('[name="iduser"]').val(response.value.id_user).end();
                $('#myModal').find('[name="username"]').val(response.value.full_name).end();
                $('#myModal').find('[name="role"]').val(response.value.role).end();
            });
           });
       });
</script>
